...
export async function approvedRecipes() { const s = await services(); const q = s.firestore.query(s.firestore.collection(s.db, 'recipes'), s.firestore.where('approved', '==', true)); return (await s.firestore.getDocs(q)).docs.map(d => ({ id: d.id, ...d.data() })); }
