...
export function exportSavedData(storage) {
  return {
    ...
    modelPipeData: readJson(storage, MODEL_PIPE_KEY, {})
  };
}
...
export function importSavedData(storage, payload, confirmReplace = () => true) {
  ...
  for (const record of incomingRecords) {
    if (recordsById.has(record.id) && !confirmReplace(`unit ${record.unitNumber} / ${record.unitModel}`)) continue;
    recordsById.set(record.id, clone(record));
    unitCount += 1;
  }
  ...
  writeJson(storage, UNIT_RECORDS_KEY, [...recordsById.values()]);
